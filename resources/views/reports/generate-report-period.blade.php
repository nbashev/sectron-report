...
<div class="container page">
	<div class="row">
		...
		<div class="col-xs-6">
			...
			<div class="row">
				<div class="col-xs-12">
					{{-- <div class="pull-right"><h4>Генериран извештај за пе</h4></div> --}}
				</div>
				<div class="col-xs-12">
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="pull-left"><h3>Извештај за период: <strong>{{$from}}</strong> - <strong>{{$to}}</strong></h3></div>
		</div>
	</div>
	<hr>
	<div class="row">
...
var format_report_date = moment($("#report_date").text()).format("dddd, DD.MM.YYYY H:M:ss");
...
$.each( reports, function( key, value ) {
	...
	if(moment(value.datetime).format("DD.MM.YYYY") == prev_date)	{
		...
		value['user_name'] = "<span class=\"hidden-datetime\" style=\"visibility: hidden\">" + value.user.name + "</span>";
	} else {
		prev_date = moment(value.datetime).format("DD.MM.YYYY");
		value["datetime_org"] = moment(value.datetime).format("DD.MM.YYYY");
